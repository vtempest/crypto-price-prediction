...
import { btcPriceCache, polymarketIntervalsCache } from "./schema";
...
export const db = drizzle(client, {
  schema: { btcPriceCache, polymarketIntervalsCache },
});
...
export async function initializeDatabase() {
  ...
  await client.execute(`
    CREATE TABLE IF NOT EXISTS polymarket_intervals_cache (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      date TEXT NOT NULL,
      interval_index INTEGER NOT NULL,
      data TEXT NOT NULL,
      created_at INTEGER NOT NULL,
      UNIQUE(date, interval_index)
    );
  `);

  await client.execute(`
    CREATE INDEX IF NOT EXISTS idx_poly_date_interval ON polymarket_intervals_cache(date, interval_index);
  `);
}
